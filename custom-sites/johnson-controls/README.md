# Johnson Controls - Demo Site (NextJS) - johnson-controls

## Overview
Johnson Controls is a manufacturing site based on the Solterra & Co. starter template. This demo site is built to showcase XM Cloud capabilities using the Content SDK.
...
2. Starting from the root of the repository navigate to site app folder
    ```cd custom-sites/johnson-controls/```
3. Copy the environment file ```.env.remote.example``` (if it exists) or create ```.env.local```
4. Edit ```.env.local``` and provide a value for ```SITECORE_EDGE_CONTEXT_ID```, ```NEXT_PUBLIC_DEFAULT_SITE_NAME=johnson-controls```, ```NEXT_PUBLIC_SITECORE_EDGE_CONTEXT_ID```, ```SITECORE_EDITING_SECRET```. (More info: [Environment variables in XM Cloud](https://doc.sitecore.com/xmc/en/developers/xm-cloud/get-the-environment-variables-for-a-site.html))

5. Install dependencies:
   from ```johnson-controls``` run ```npm install```
...
6. Provide Editing host name  ```johnson-controls``` as per xmcloud.build.json
