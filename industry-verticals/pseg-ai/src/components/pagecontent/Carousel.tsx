...
export const Pseg = (props: CarouselComponentProps): JSX.Element => {
  ...
  return (
    <section className={`component carousel pseg-hero ${sxaStyles}`} id={id ? id : undefined}>
      <div className="carousel-inner">
        {items.map((item, i) => (
          <div key={i} className={'carousel-item ' + (i == index ? 'active' : '')}>
            <NextImage
            ...
            <div className="pseg-hero-grid">
              <div className="pseg-hero-copy">
                <h1 className="display-6 fw-bold">
                  <Text field={item.fields.Title} />
                </h1>
                <RichText field={item.fields.Text} />
                {(isPageEditing || item.fields?.Link?.value?.href) && (
                  <Link field={item.fields.Link} className="button button-simple" />
                )}
              </div>
              <div />
              <div className="pseg-hero-login">
                <h2>Login to My Account</h2>
                <a
                  className="button button-main"
                  href="https://nj.myaccount.pseg.com/user/login"
                >
                  Login
                </a>
                <a
                  className="button button-secondary"
                  href="https://nj.myaccount.pseg.com/user/registration"
                >
                  Register
                </a>
              </div>
