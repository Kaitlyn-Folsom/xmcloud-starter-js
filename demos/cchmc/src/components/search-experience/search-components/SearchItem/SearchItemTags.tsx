      ...
      <div className={cn('flex flex-wrap gap-2 mb-4', className)} {...props}>
        {Array.isArray(tags.value) ? (
          tags.value.map((tag, index) => (
            <span key={index} className="px-2 py-1 text-xs bg-secondary text-foreground rounded-none">
              {tag}
            </span>
          ))
        ) : (
          <Text
            ...
            tag="span"
            className="px-2 py-1 text-xs bg-secondary text-foreground rounded-none"
          />
        )}
      </div>
    )
