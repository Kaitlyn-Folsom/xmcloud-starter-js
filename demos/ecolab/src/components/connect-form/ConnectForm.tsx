...
export const Default: React.FC<MyChartFormProps> = () => {
    ...
    if (shouldSendIdentityEvent) {
      ...
      await identity({
        ...
        if (error?.status !== 404 && error?.status !== 0) {
          console.debug('IDENTITY event error:', error);
        }
      });
    }
  ...
  return (
    <section className="component mychart-form bg-gray-100 w-full px-4 py-16">
      <div className="mx-auto max-w-2xl">
        <h1 className="font-heading text-center text-foreground mb-8 text-3xl font-semibold leading-tight tracking-tight md:text-4xl">
